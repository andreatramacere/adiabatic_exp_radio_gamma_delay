...
def perform_mc(x=None, write_data=True):
    if x is None:
    ...
    if write_data:
        ...
        highlights[1] = (np.array(highlights[0]) + delta_t).tolist()
        plot_lcs(gev_3c273, radio_3c273, highlights=highlights, figsize=(10,5))
        result = find_flare_amplitudes(_v.x, plot=True, detrend=True, flaring_periods=fp)
...
perform_mc(x=7436, write_data=False)





xyz123 # intentional mistake to prevent cell from executing
